...
export default function CustomerProfilePage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const [custRes, stmtRes] = await Promise.all([
                customersApi.getById(id, storeId),
            ...
            console.error('Customer profile load error details:');
    ...
    const handleRecordPayment = async (e: React.FormEvent) => {
        ...
        try {
            const storeId = localStorage.getItem('storeId') || '';
            await customersApi.recordPayment(storeId, id, {
    ...
    if (!customer) {
        return (
            <div className="flex flex-col items-center justify-center h-96 space-y-4">
                ...
                <Button onClick={() => router.push('/store/customers')}>Go Back</Button>
            </div>
        );
    }
                ...
                <div className="lg:col-span-1 space-y-6">
                        ...
                        <div className="flex items-center gap-4 mb-6">
                            ...
                            <div>
                                ...
                                <span className={`px-3 py-1 rounded-full text-xs font-bold inline-block mt-1 ${
                                    customer.customerType === 'WHOLESALE' ? 'bg-emerald-50 text-emerald-600' : 'bg-blue-50 text-blue-600'
                                }`}>
                    ...
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="card-premium p-6">
                        ...
                        <div className="space-y-4">
                            <BalanceDisplay 
                                balance={statement?.currentBalance ?? customer.outstandingBalance ?? 0}
                                partyType="CUSTOMER"
                                size="sm"
                                showBreakdown={true}
